...
[TestClass]
public class WithNotifyWhatsappCommandWithUrlMediaType
{
    ...
    public WithNotifyWhatsappCommandWithUrlMediaType()
    {
        // Arrange
        ...
        _command = _fixture.Build<NotifyWhatsappCommand>()
            .With(x => x.MessageType, () => MessageType.UrlMedia)
            .Create();
    ...
    public Task IEnvelopEventServiceShouldPublishAMessageWithCorrectValues()
       => _publishService.Received().Publish(
           Arg.Is<SendMediaWhatsAppEvent>(p => AssertWhatsappMediaEvent(p)),
    ...
    private bool AssertWhatsappMediaEvent(SendMediaWhatsAppEvent @event)
    {
        @event.NotificationId.Should().NotBeEmpty();
        @event.To.Should().Be(_command.RecipientNumber);
        @event.MediaUrl.Should().Be(_command.Message);
