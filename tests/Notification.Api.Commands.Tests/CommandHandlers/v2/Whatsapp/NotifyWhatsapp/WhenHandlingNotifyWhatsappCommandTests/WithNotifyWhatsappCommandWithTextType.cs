...
[TestClass]
public class WithNotifyWhatsappCommandWithTextType
{
    ...
    public WithNotifyWhatsappCommandWithTextType()
    {
        // Arrange
        ...
        _command = _fixture.Build<NotifyWhatsappCommand>()
            .With(x => x.MessageType, () => MessageType.Text)
            .Create();
    ...
    public Task IEnvelopEventServiceShouldPublishAMessageWithCorrectValues()
        => _publishService.Received().Publish(
            Arg.Is<SendTextWhatsAppEvent>(p => AssertWhatsappTextEvent(p)),
    ...
    private bool AssertWhatsappTextEvent(SendTextWhatsAppEvent @event)
    {
