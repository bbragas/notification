...
[TestClass]
public class WithANotifySmsCommandToBeFiredImmediately
{
    ...
    public WithANotifySmsCommandToBeFiredImmediately()
    {
        // Arrange
        ...
        _mediator.Send(Arg.Any<GetAllBlacklistQueryRequest>(), Arg.Any<CancellationToken>())
                .Returns(_response);
        _command = _fixture.Build<NotifySmsCommand>()
    ...
    => _publishService.Received().Publish(
        Arg.Is<SendSmsEvent>(p => AssertSmsEvent(p)),
    ...
    private bool AssertSmsEvent(SendSmsEvent @event)
    {
        @event.Id.Should().NotBeEmpty();
        @event.To.Should().Be(_command.PhoneNumber);
        @event.Text.Should().Be(_command.Message);
        @event.Campaign.Should().Be(_command.Campaign);
